...
jsPsych.data = (function() {
  ...
  module.displayData = function(format) {
    ...
    if (format != "json" && format != "csv") {
      console.log('[JSPSYCH] - Invalid format declared for displayData function. Using json as default.');
      format = "json";
    }
  ...
  module.repeat = function(array, repetitions, unpack) {
    ...
    if (!arr_isArray) {
      if (!rep_isArray) {
        ...
        console.log('[JSPSYCH] - Unclear parameters given to randomization.repeat. Multiple set sizes specified, but only one item exists to sample. Proceeding using the first set size.');
      }
    } else {
      if (!rep_isArray) {
        var reps = [];
        ...
        if (array.length != repetitions.length) {
          console.warning('[JSPSYCH] - Unclear parameters given to randomization.repeat. Items and repetitions are unequal lengths. Behavior may not be as expected.');
...
jsPsych.pluginAPI = (function() {
  ...
  module.getKeyboardResponse = function(parameters) {
    ...
    if (parameters.rt_method != 'date' && parameters.rt_method != 'performance' && parameters.rt_method != 'audio') {
      console.log('[JSYPSYCH] - Invalid RT method specified in getKeyboardResponse. Defaulting to "date" method.');
