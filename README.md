...
The Operation Span has a basic implementation, with no testing or integration of results into a database. 
The implementations were directly derived from the original E-Prime experiments, with some modifications to instructions and styling to fit our needs.
...
Support for Docker is available, and can be enabled by installing Docker: https://www.docker.com/. 
Once installed, point Docker to the Dockerfile at the root of the repository to build. 
Additionally, a python utility script `build_docker_container.py` encapsulates several commonly repeated commands and options.

# Backend
## Overview
The backend portion of the Oswald Span Tasks, located under `/oswald_operation_span/backend`, 
encapsulates the Cherrypy web server that hosts the span task code for the web. Backend responsibilities include:
